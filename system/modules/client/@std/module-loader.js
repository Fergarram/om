...
async function initializeBlobModuleLoader() {
	...
	style_tags.forEach((style) => {
		...
		if (__blob_styles__.has(style_module_name) || remote_styles.has(style_module_name)) {
		...
		if (!content) {
			...
			remote_styles.set(style_module_name, remote_url);
			return;
		} else {
			// Both remote and content exist - store local, compare later
			__blob_styles__.set(style_module_name, content);
			remote_styles.set(style_module_name, remote_url);
			return;
		}
	});
	...
	for (const [style_name, remote_url] of remote_styles) {
		try {
			let remote_content;
			...
			if (__blob_styles__.has(style_name)) {
				...
				const local_content = __blob_styles__.get(style_name);
				if (remote_content.trim() !== local_content.trim()) {
					if (__blob_module_loader_settings__.prefers_remote_modules) {
						...
						__blob_styles__.set(style_name, remote_content); // Use remote content
					} else {
				...
				__blob_styles__.set(style_name, remote_content);
	...
	__blob_styles__.forEach((content, style_name) => {
		...
		const remote_url = remote_styles.get(style_name) || null;
		...
		window.__blob_style_map__.set(style_name, metadata);
	});
