...
async function getFileIntegrity(inputFileStream) {
  ...
  let currentBlock = [];
  await streamPromises.pipeline(inputFileStream, new stream.PassThrough({
    decodeStrings: false,
...
class Filesystem {
  ...
  searchNodeFromPath(p) {
    p = path2.relative(this.src, p);
    if (!p) {
      return this.header;
    }
    const name = path2.basename(p);
    const node = this.searchNodeFromDirectory(path2.dirname(p));
    if (!node.files) {
  ...
  async insertFile(p, streamGenerator, shouldUnpack, file, options = {}) {
    const dirNode = this.searchNodeFromPath(path2.dirname(p));
    const node = this.searchNodeFromPath(p);
    ...
    if (transformed) {
      const tmpdir = await wrappedFs.mkdtemp(path2.join(os.tmpdir(), "asar-"));
      const tmpfile = path2.join(tmpdir, path2.basename(p));
      const out = wrappedFs.createWriteStream(tmpfile);
      await stream2.pipeline(streamGenerator(), transformed, out);
      file.transformed = {
        path: tmpfile,
  ...
  insertLink(p, shouldUnpack, parentPath = wrappedFs.realpathSync(path2.dirname(p)), symlink = wrappedFs.readlinkSync(p), src = wrappedFs.realpathSync(this.src)) {
  ...
  resolveLink(src, parentPath, symlink) {
    const target = path2.join(parentPath, symlink);
    const link = path2.relative(src, target);
    return link;
  }
  listFiles(options) {
    const files = [];
    const fillFilesFromMetadata = function(basePath, metadata) {
      ...
      for (const [childPath, childMetadata] of Object.entries(metadata.files)) {
        const fullPath = path2.join(basePath, childPath);
  ...
  getNode(p, followLinks = true) {
    ...
    if ("link" in node && followLinks) {
      return this.getNode(path2.join(node.link, name));
    }
    if (name) {
      return node.files[name];
    } else {
...
var writeFileListToStream = async function(dest, filesystem, out, lists, metadata) {
  ...
  for (const file of files) {
    if (file.unpack) {
      const filename = path3.relative(filesystem.getRootPath(), file.filename);
...
async function streamFilesystem(dest, filesystem, lists) {
  ...
  for await (const file of files) {
    if (file.unpack) {
      const targetFile = path3.join(`${dest}.unpacked`, file.filename);
      await wrappedFs.mkdirp(path3.dirname(targetFile));
      const writeStream = wrappedFs.createWriteStream(targetFile, { mode: file.mode });
      await stream3.pipeline(file.streamGenerator(), writeStream);
    } else {
...
function readFileSync(filesystem, filename, info) {
  ...
  if (info.unpacked) {
    buffer = wrappedFs.readFileSync(path3.join(`${filesystem.getRootPath()}.unpacked`, filename));
  } else {
...
async function createSymlink(dest, filepath, link) {
  await wrappedFs.mkdirp(path3.join(`${dest}.unpacked`, path3.dirname(filepath)));
  await wrappedFs.symlink(link, path3.join(`${dest}.unpacked`, filepath)).catch(async (error) => {
...
`).map((line) => {
  ...
  const handleFile = async function(filename) {
    ...
    switch (file.type) {
      case "directory":
        shouldUnpack = shouldUnpackPath(path5.relative(src, filename), undefined, options.unpackDir);
  ...
  const insertsDone = async function() {
    await wrappedFs.mkdirp(path5.dirname(dest));
  ...
  const handleFile = async function(stream5) {
    ...
    switch (type) {
      ...
      case "link":
        ...
        filesystem.insertLink(filename, stream5.unpacked, path5.dirname(filename), stream5.symlink, src);
        break;
    }
    return Promise.resolve();
  };
