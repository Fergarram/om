...
export type StaticTagFunction = (...args: TagArgs) => string;

export type TagsProxy = {
	[key: string]: TagFunction;
};

export type StaticTagsProxy = {
	[key: string]: StaticTagFunction;
};
...
export function useStaticTags(): StaticTagsProxy {
	return new Proxy({}, { get: staticTagGenerator });
}

export function useTags(options?: string | UseTagsOptions): TagsProxy | StaticTagsProxy {
