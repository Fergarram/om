...
type SB_Object = {
	...
	mouse_pressed?: (self: SB_Instance, button: number) => void;
	mouse_released?: (self: SB_Instance, button: number) => void;
	mouse_wheel?: (self: SB_Instance, delta: number) => void;
	touch_start?: (self: SB_Instance, touch_id: number, x: number, y: number) => void;
	touch_move?: (self: SB_Instance, touch_id: number, x: number, y: number) => void;
	touch_end?: (self: SB_Instance, touch_id: number, x: number, y: number) => void;
...
function create_game(config: SB_Config) {
	...
	async function run_game(start_callback: (gm: SB_Game) => void) {
		...
		canvas.addEventListener("mousedown", (e) => {
			...
			Object.values(room.instances).forEach((instance) => {
				...
				if (point_in_instance(instance, gm.mouse_x, gm.mouse_y)) {
					if (obj.mouse_pressed) {
						obj.mouse_pressed(instance, e.button);
					}
				}
			});
		});
			...
			Object.values(room.instances).forEach((instance) => {
				...
				if (point_in_instance(instance, gm.mouse_x, gm.mouse_y)) {
					if (obj.mouse_released) {
						obj.mouse_released(instance, e.button);
					}
				}
			});
		});
			...
			Array.from(e.changedTouches).forEach((touch) => {
				...
				Object.values(room.instances).forEach((instance) => {
					...
					if (obj.touch_start && point_in_instance(instance, touch_x, touch_y)) {
						obj.touch_start(instance, touch.identifier, touch_x, touch_y);
					}
				});
			});
		});
			...
			Array.from(e.changedTouches).forEach((touch) => {
				...
				Object.values(room.instances).forEach((instance) => {
					...
					if (obj.touch_move && point_in_instance(instance, touch_x, touch_y)) {
						obj.touch_move(instance, touch.identifier, touch_x, touch_y);
					}
				});
			});
		});
			...
			Array.from(e.changedTouches).forEach((touch) => {
				...
				Object.values(room.instances).forEach((instance) => {
					...
					if (obj.touch_end && point_in_instance(instance, touch_point.x, touch_point.y)) {
						obj.touch_end(instance, touch.identifier, touch_point.x, touch_point.y);
					}
				});
		...
		function game_loop(): void {
			...
			const room = gm.rooms[gm.current_room];
			const frame_interval = 1000 / room.room_speed;
			const current_time = performance.now();
			const dt = current_time - last_frame_time;

			// Update frame times buffer for averaging
			frame_times.push(dt);
			...
			// WARNING: Using the filter below may result in bad performance.
			if (!gm.config.shape_smoothing_enabled && gm.ctx.filter === "none") {
				gm.ctx.filter = `url('data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg"><filter id="f" color-interpolation-filters="sRGB"><feComponentTransfer><feFuncA type="discrete" tableValues="0 1"/></feComponentTransfer></filter></svg>#f')`;
			}
			...
			if (gm.config.debug) {
				...
				gm.ctx.fillStyle = gm.config.debug_color || "#FF0000";
				gm.ctx.fillText(`Avg: ${avg_frame_time.toFixed(2)}ms (${fps.toFixed(1)} FPS)`, 10, 20);
				gm.ctx.restore();
			}
			...
			const elapsed = performance.now() - current_time;
			const next_frame_delay = Math.max(0, frame_interval - elapsed);

			// Schedule next frame
			setTimeout(game_loop, next_frame_delay);
		}
	...
	function draw_collision_mask(instance: SB_Instance): void {
		const ctx = gm.ctx;
		ctx.save();
		ctx.strokeStyle = gm.config?.debug_color || "#FF0000";
		ctx.lineWidth = 2;
