...
import { finish, isUserTyping } from "@/monzon/src/lib/utils";
...
export function useSelect(...args: SelectArgs): Promise<any> {
	return new Promise((resolve) => {
		...
		backdrop_el = tags.div({
			class: tw("component-select-backdrop", {
		...
		const select_el = tags.div(
			{
				style: `max-height: ${max_height}; ${
					typeof width === "number" ? `width: ${width}px;` : width !== "fit" && width !== "parent" ? `width: ${width};` : ""
				}`,
				tabIndex: -1,
				...rest_props,
...
export function SeparatorItem() {
	return tags.div({ class: "component-select-separator" });
}
