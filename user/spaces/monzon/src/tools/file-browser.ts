...
import type { DirectoryEntry } from "@/monzon/src/lib/types";
import {
	...
	isSoundFile,
	shortcut,
} from "@/monzon/src/lib/utils";
import { Button } from "@/monzon/src/ui/button";
import { liftMirror, mountWindow, openedWindows, useDesktop } from "@/monzon/src/ui/desktop";
import { playGame } from "@/monzon/src/tools/runner";
import { SelectItem, SeparatorItem, useSelect } from "@/monzon/src/ui/select";
import { WindowFrame } from "@/monzon/src/ui/window-frame";
import { AppSettings, AppState, findEntry, readDirectoryEntries, syncWorkingDirectory } from "@/monzon/src/lib/state";
import { openCodeEditor } from "./code-editor";
import { openImageViewer } from "./image-viewer";
import { openSoundViewer } from "./sound-viewer";
...
async function showFileMenu(e: MouseEvent, entry?: DirectoryEntry) {
	...
	await useSelect(
		...
		entry && entry.type === "project" ? SeparatorItem() : null,
		...
		entry ? SeparatorItem() : null,
		SelectItem({ value: "show" }, "Show in file explorer"),
		!is_in_trash && ((entry && entry.type !== "project") || !entry) ? SeparatorItem() : null,
							...
							Button(
								{
									...
									class: tw("w-full uppercase"),
									onclick() {
										sys.win.open_in_browser("https://stormborn.pro");
									},
								},
