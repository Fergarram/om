	...
	async function callAssistant(just_estimate: boolean = false) {
		...
		try {
			...
			if (AppSettings.assistants[selected_assistant].instructions) {
				...
				let stormborn_source = await sys.file.read(
					`${basepath}/user/spaces/monzon/files/engines/${AppSettings.default_version}/stormborn.ts`,
				);
				const instructions_fullpath = `${basepath}/user/spaces/monzon/files/engines/${AppSettings.default_version}/${AppSettings.assistants[selected_assistant].instructions}`;
					...
					button(
						...
						() => {
							if (token_display_mode === "tokens") return `${estimated_tokens} tokens`;
								...
								return `${(3 * (estimated_tokens / 1000000)).toFixed(2)} USD`;
										...
										SelectItem(
											{
												value: id,
												class: tw(
													"overflow-hidden whitespace-nowrap !justify-start !gap-1 max-w-full relative",
												),
											},
