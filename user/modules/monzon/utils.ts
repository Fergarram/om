import type { ShortcutKey, ShortcutModifier } from "@/monzon/types";
...
export function isValidProjectFolder(item: FileItem): boolean {
	...
	if (folder_files.includes("package.json") && has_public_dir) {
		// Project type (has package.json and public dir)
		return true;
	} else if (folder_files.includes("template.html") && folder_files.includes("stormborn.ts")) {
		// Legacy type (has template.html and stormborn.ts)
		return true;
	} else if (
		...
		// Prototype type (has index.html but no template.html)
		return true;
	}
