...
Options:
  ...
  -a, --abi             Print the Node ABI version.`;
	...
	if (process.platform === "win32") {
		rootDir = path.dirname(path.dirname(appPath));
	} else {
		console.log("appPath", appPath, process.cwd());
		rootDir = true
	...
	if (!fs.existsSync(systemPath)) {
		showErrorMessage(`Cannot find system directory at ${systemPath}`);
		process.exit(1);
	}

	await loadApplicationPackage(systemPath);
}
